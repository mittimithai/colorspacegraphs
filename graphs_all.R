...
with(spectrum.df, {
 	if( rgl.cur() == 0 ) {
    rgl.open()
    par3d(zoom= 0.7206817, userMatrix=matrix(c(0.965923726558685,0.205491006374359,0.157368451356888,0,0,0.608006477355957,-0.79393208026886,0,-0.258826941251755,0.766877830028534,0.587287902832031,0,0.0923089283171894,0.189200585444115,6.80704925759557e-09,1),4,4),windowRect = c( 1581, 44, 3581, 1440 ) )
    rgl.bg(color = "black" )
  }
 ...
 # rgl.viewpoint(theta = 0, phi = -130, zoom = 1)
  ...
  axes3d( edges=c("x--", "y+-", "z--"), col="white", cex=1.5,nticks=5)
    ...
    text3d(L,M,S,wl.labels,adj=c(0,0),depth_test="always", cex=2)
...
with(spectrum.df, {
  ...
  text3d(X,Y,Z,wl.labels,adj=c(0,0),depth_test="always",cex=2)
...
geom_text(data=g.xy.df %>% filter(wavelength >= 525), aes(x,y,color=rgb(R,G,B),label=wl.labels),hjust=-0.5, vjust=0, cex=label.size)+
geom_text(data=g.xy.df %>% filter(!(wavelength >= 525),wavelength>450), aes(x,y,color=rgb(R,G,B),label=wl.labels),hjust=1.3, vjust=0, cex=label.size)+
xlim(c(0,0.7))+
